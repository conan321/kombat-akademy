...
function setPlatform(platform, targetContainers) {
	...
	containers.forEach(c => {
		...
		if (inputs.length == 0) {
			isPlainText = true;
			inputs = c.textContent.match(/ |,|\+|HOLD|DELAY|RELEASE|OR|\(AIR\)|DURING CERTAIN MOVES|DURING ENHANCED MOVES|DURING START-UP|DURING SLIDE|DURING BOMBS|DURING RUSH|DURING ENHANCED RUSH|DURING DELAY KNIFE TOSS|DURING|END OF MOVE|NEAR CORNER|WHILE PRONE|WHILE IN AIR|1\/2\/3\4 x5|THROW|KAMEO|SS|EX|U\+B|U\+F|D\+B|D\+F|B|U|D|F|1|2|3|4/gi);
			c.innerHTML = '';
		}
